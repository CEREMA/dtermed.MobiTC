MOBITC_Stat_RLS<-function(distTDC,numdateTDC,IC,datex,numdateprosp)
{
if (length(distTDC)>1)
{
	model1=lm(distTDC ~ as.vector(numdateTDC))
	ecart_type= sd(distTDC)
	err=abs(distTDC-(coef(model1)[2]*as.vector(numdateTDC)+coef(model1)[1]))

	poids=seq(1,length(distTDC))
...
if (length(IC)>0)
{
	for (ic in 1 : length(IC))
	{
		...
		colnames(res)[2*ic]=paste("RLS",IC[ic],"lwr",sep="")
		colnames(res)[2*ic+1]=paste("RLS",IC[ic],"up",sep="")
	}
} else {
	if (length(distTDC)>1)
	{
