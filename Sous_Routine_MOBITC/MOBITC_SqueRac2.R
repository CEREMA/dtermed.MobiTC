...
MOBITC_SqueRac2<-function(chem_mobitc,fichier_sque,distrac)
{
  ...
  tabsque0=data.frame(sapply(slot(SP, 'lines'), function(x) slot(x, 'ID')))
  tabsque=data.frame(tabsque0,rep(nom_projet,nrow(tabsque0)))
  Squelette2 = SpatialLinesDataFrame(SP, data=tabsque)
  
  ###############
  names(Squelette2)[1] <- "NAxe"
  names(Squelette2)[2] <- "Loc"
