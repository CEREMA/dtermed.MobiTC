MOBITC_Aide<-function(chem_mobitc)
{
  aide_pres=div(h3("Bienvenue dans MobiTC"),
				...
				p(),
				p(span(style="color:green","Les 4 traits de côtes sont fournis uniquement dans le cadre du tutoriel. Ne pas les utiliser pour une étude de l'évoultion du trait de côte de ce secteur."))
				)
...
médiane. Pour cela le principe de la squelettisation a été retenu. "),
				...
				h3("Attention"),
			    p("Une fois l'enveloppe produite, il faut s'assurer que l'enveloppe ne contient pas d'îlot. Ceci peut être réalisée en parcourant l'enveloppe dans la carte résultat. Si il y a des ilots il faut les supprimer avec un logiciel SIG. 
...
au maximum, perpendiculaires à la côte (léger biais sinon). ."),
  ...
  aide_evol=div(h3("Objectifs"),
				...
				h3("Sorties :"),
				p("A l'issue de cette étape, un répertoire Graph est créé dans le répertoire de travail. Il contient trace par trace, tous éléments pour que le logiciel effectue les sorties graphiques des menus suivants."),
				p("Les calculs sont réalisés pour 4 périodes, avec toutes les traits de côte (toutes dates), avec les traits de côtes post 1950, avec les traits de côtes post 2000 et avec les traits de côtes post 2010"),
				p("4 fichiers txt sont aussi créés dans le répertoire de travail. Ils contiennent les taux d'évolution et leurs intervalles de confiance pour chaque loi statistique."),
  ...
  aide_histo=div(h3("Objectifs"),
                  p(paste("Dans ce menu, il est possible de réaliser un premier rendu sous forme d'histogrammes des taux d'évoultion du trait de côte.")),
				  h3("Entrées :"),
				  p("Commencez par actualiser le menu et pensez à actualiser les noms des fichiers si ils ont été modifiés."),
				  h4("Choisir les fichiers de l'évolution (-MobiTC.txt)"),
				  p("Choisir le ou les fichiers sur lesquels seront basés le calcul des histogrammes. Pour éditer les rapports finaux il faut choisir les 4 fichiers réalisés à l'étape précédente."),
  ...
  aide_rapport=div(h3("Objectifs"),
                      p(paste("Dans ce menu, des rapports html résumant l'ensemble des calculs menés sont édités")),
					  h3("Entrées :"),
				  p("Commencez par actualiser le menu et pensez à actualiser les noms des fichiers si ils ont été modifiés."),
				  h4("Exporter les rapports complets par trace"),
					   h3("Sorties :"),
				  p("A l'issue de cette étape, un répertoire Rapport est créé dans le répertoire de travail. Il contient tous les rapports html"),
				  p("le fichier trace a été mis à jour pour ne contenir que les traces pour lesquelles un rapport a été édité."),
