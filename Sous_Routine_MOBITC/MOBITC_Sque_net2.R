...
MOBITC_Sque_net2<-function(chem_mobitc,chemin_rep_travail,fichier_voro)
{
  ...
  #lecture dans le fichier paramètre
...
for (ienv in 1 : length(Enveloppe))
    ...
    # boucle pour supprimer les extrêmités par des cercles
    rempli=1
    tour=0
    while (rempli==1)
      {
      ...
      if (length(SP)>0)
        {
		  ...
		  #pb si plusieurs morceaux
			...
			# Creation des mini-cercles
			for (ires in 1:length(reseau@nb))
			{
			  # Recuperation des axes finaux
    ...
    #derniere verif et suppression des jonctions e 3 morceaux
    tokeep=data.frame()
    ...
    # Verification que les squelettes ne soient pas trop petits
    ...
    tabsque0=data.frame(sapply(slot(SLDF, 'lines'), function(x) slot(x, 'ID')))
    tabsque=data.frame(tabsque0,rep(nom_projet,nrow(tabsque0)))
    SLDFF = SpatialLinesDataFrame(SLDF, data=tabsque)
