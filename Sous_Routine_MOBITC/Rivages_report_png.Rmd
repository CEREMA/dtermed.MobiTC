...
assign(feuille.name[1],MOBITC_Export_Graph_1p1(params$chem_mobitc,params$chemin_rep_travail,params$fichier_sque,params$fichier_trace,
                              params$fichier_intersectionv1,fichier_evolution[1],params$NAxe,params$NTrace))
...
assign(feuille.name[k],MOBITC_Export_Graph_1p1(params$chem_mobitc,params$chemin_rep_travail,params$fichier_sque,params$fichier_trace,
                              params$fichier_intersectionv1,fichier_evolution[k],params$NAxe,params$NTrace))
}
...
l=leaflet()%>%
  ...
  addPolygons(data=Histo1deg,fillColor = ~pal1(WLS),  weight = 1,  opacity = 1,  color = "white",  fillOpacity = 0.9,
              ...
              group="Taux - Toutes dates")%>%
  addPolygons(data=Histo2deg,fillColor = ~pal1(WLS),  weight = 1,  opacity = 1,  color = "white",  fillOpacity = 0.9,label = ~as.character(Histo2deg$WLS),highlightOptions = highlightOptions(color = "black", weight = 2,  bringToFront = TRUE),group="Taux - Post 1950")%>%
  addPolygons(data=Histo3deg,fillColor = ~pal1(WLS),  weight = 1,  opacity = 1,  color = "white",  fillOpacity = 0.9,
             label = ~as.character(Histo3deg$WLS),highlightOptions = highlightOptions(color = "black", weight = 2,  bringToFront = TRUE),group="Taux - Post 2000")%>%
  addPolygons(data=Histo4deg,fillColor = ~pal1(WLS),  weight = 1,  opacity = 1,  color = "white",  fillOpacity = 0.9,
              label = ~as.character(Histo4deg$WLS),highlightOptions = highlightOptions(color = "black", weight = 2,  bringToFront = TRUE),group="Taux - Post 2010")%>%
  ...
  setView(lng = xmoy, lat = ymoy, zoom = 16)
				...
				l %>%
				  addLayersControl(baseGroups = names(esri),overlayGroups = c("Ligne de base","Trace","Taux - Toutes dates","Taux - Post 1950","Taux - Post 2000","Taux - Post 2010"),options = layersControlOptions(collapsed = FALSE)) %>%
				  hideGroup("Taux - Post 1950") %>%
				  hideGroup("Taux - Post 2000") %>%
				  hideGroup("Taux - Post 2010") %>%
...
```{r, echo=FALSE, warning=FALSE, message = FALSE, fig.width=6,fig.height=4.5,out.width=200,out.height=150}
  nomdirgraphpng=paste(chemin_rep_travail,"\\Graph_png",sep="")
  if (file.exists(nomdirgraphpng)=="FALSE"){dir.create(nomdirgraphpng)}

  nom_fiche=paste0("\\Graph_png\\",substr(params$fichier_intersectionv1,1,nchar(params$fichier_intersectionv1)-4),"-Graph-TDC-NAxe",params$NAxe,"-NTrace",params$NTrace,".png")
...
for (k in 1: nbfeuille) {
  ...
  nomcourt=c("toutesdates",paste0("post",datecoup))
  nom_fiche=paste0("\\Graph_png\\",substr(params$fichier_intersectionv1,1,nchar(params$fichier_intersectionv1)-4),"-Graph-",nomcourt,"-NAxe",params$NAxe,"-NTrace",params$NTrace,".png")
...
```{r echo=FALSE,warning=FALSE,fig.width=6,fig.height=4.5,out.width=200,out.height=150}
 nom_fiche=paste0("\\Graph_png\\",substr(params$fichier_intersectionv1,1,nchar(params$fichier_intersectionv1)-4),"-Graph-expliTDC-NAxe",params$NAxe,"-NTrace",params$NTrace,".png")
