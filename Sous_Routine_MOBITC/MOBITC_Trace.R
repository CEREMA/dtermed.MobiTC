...
#chemin_rep_travail="C:\\0_ENCOURS\\TPM\\Erosion\\Sque"
# fichier_squef="20180124T111059--Env-C010-T0250-Sque-C001"
#fichier_squef="N_artificialisation_TPM_est"
# 
...
MOBITC_Trace<-function(chem_mobitc,chemin_rep_travail,fichier_squef,disttrace,longtrace)
{
  ...
  for (i in 1 : length(sque))
  {
    ...
    if (length(sque$Loc)>0)
    {tab=data.frame(
      NAxe=rep(sque$NAxe[i],length(transect$x0)),
      NTrace=seq(1,length(transect$x0)),
      Loc=rep(sque$Loc[i],length(transect$x0)))
    }
    else
    {
    tab=data.frame(
