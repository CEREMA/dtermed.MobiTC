...
MOBITC_Export_Histo<-function(chem_mobitc,chemin_rep_travail,fichier_trace,fichier_mobitc,largeur_histo,longueur_histo,tronqu_histo,taux_histo)
{
  ...
  Trace = readOGR(dsnlayer,fichier_trace)
   ...
   for (k in 1:length(fichier_mobitc))
  {
  
  #lecture du fichier résultat MobiTC
  chem_resultat=paste0(chemin_rep_travail,"\\",fichier_mobitc[k])
  print(chem_resultat)
  tab=read.table(chem_resultat,sep="\t",header=TRUE,row.names = NULL)
  if (is.numeric(tab$Loc)==TRUE)
  {
    tab$Loc=as.character(formatC(as.numeric(tab$Loc), width = 5, format = "d", flag = "0"))
  }
  ...
  for (j in 1:length(taux_histo))
  {
    ...
    #nom export histo
    nom_export=paste0(substr(fichier_mobitc[k],1,nchar(fichier_mobitc[k])-10),"histo-",taux_histo[j])
    ...
    for (i in 1:nrow(tab))
    {
      ...
      nom_png=paste0("Graph_WLS\\",substr(fichier_mobitc[k],1,nchar(fichier_mobitc[k])-10),"Graph-NAxe",tab$NAxe[i],"-NTrace",tab$NTrace[i],"-Loc",tab$Loc[i],".png")
      
      data=data.frame(tab[i,],Graph_WLS=nom_png)
